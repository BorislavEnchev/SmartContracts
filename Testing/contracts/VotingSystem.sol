...
contract VotingSystem { 
    ...
    * @param executed Whether the proposal has been executed */ 
    ...
    function createProposal(string memory _description, uint256 _votingPeriod) external onlyOwner { 
    ...
    function executeProposal(uint256 _proposalId) external onlyOwner { 
        if (_proposalId >= proposalCount) revert InvalidProposal(); 
        if (block.timestamp < proposals[_proposalId].endTime) revert VotingNotEnded();
